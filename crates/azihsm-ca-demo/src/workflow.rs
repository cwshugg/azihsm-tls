//! Create, retry, show, and confirmed named-key deletion workflows.

use crate::cli::{
    CreateArgs, DeleteKeyArgs, OutputArgs, RetryArgs, validate_ca_url, validate_cn,
    validate_key_name, validate_sans,
};
use crate::files;
...
use crate::{Error, ErrorClass, Result, csr, verify};
...
pub fn create(args: CreateArgs) -> Result<()> {
    ...
    enroll_and_publish(&args.output_dir, &metadata, &spki, &csr_der)?;
...
fn deletion_status(output_dir: &Path, metadata: &RequestMetadata) -> Result<&'static str> {
...
fn validate_stored_request(output_dir: &Path, metadata: &RequestMetadata) -> Result<Vec<u8>> {
...
fn load_request(output_dir: &Path) -> Result<RequestMetadata> {
